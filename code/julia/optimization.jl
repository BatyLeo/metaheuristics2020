...
function build_LP(dataset::Dataset; log_file_path="log.txt")
    ...
    delta = @variable(model, 0 <= delta[1:number_targets+1] <= 1)
    x = @variable(model, 0 <= x[1:E_com_length] <= 1)
